...
export default function CarritoPage() {
  ...
  if (items.length === 0) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <h1 className="text-2xl font-bold text-[#4410b9] mb-2">Tu carrito está vacío</h1>
        <p className="text-gray-500 mb-6">Agrega productos para comenzar tu pedido.</p>
        <Link href="/" className="bg-[#4410b9] text-white font-semibold px-6 py-3 rounded-xl hover:bg-[#3509a0] transition-colors inline-block">
          Ver productos
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold text-[#4410b9] mb-6">Mi carrito ({items.length} {items.length === 1 ? 'producto' : 'productos'})</h1>
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          {items.map((item) => (
            <div key={item.producto.id} className="bg-white border border-gray-200 rounded-2xl p-4 flex gap-4">
              ...
              <div className="flex-1 min-w-0">
                <Link href={`/productos/${item.producto.slug}`} className="font-medium text-[#4410b9] hover:text-gray-600 text-sm line-clamp-2">{item.producto.nombre}</Link>
                <p className="text-gray-400 text-xs mt-0.5">{item.producto.subcategoria}</p>
                <div className="flex items-center justify-between mt-3">
                  <div className="flex items-center bg-gray-100 rounded-lg">
                    <button onClick={() => updateCantidad(item.producto.id, item.cantidad - 1)} className="w-8 h-8 flex items-center justify-center text-gray-600 hover:text-[#4410b9]"><FiMinus size={12}/></button>
                    <span className="w-8 text-center text-sm font-semibold">{item.cantidad}</span>
                    <button onClick={() => updateCantidad(item.producto.id, item.cantidad + 1)} className="w-8 h-8 flex items-center justify-center text-gray-600 hover:text-[#4410b9]"><FiPlus size={12}/></button>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="font-bold text-[#4410b9]">{fmt(item.producto.precio * item.cantidad)}</span>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white border border-gray-200 rounded-2xl p-5 sticky top-32">
            <h2 className="font-bold text-[#4410b9] mb-4">Resumen del pedido</h2>
            ...
            <div className="border-t border-gray-100 pt-3 mb-5">
              <div className="flex justify-between font-bold text-[#4410b9] text-lg">
                <span>Total</span>
